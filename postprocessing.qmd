...
```{r}
load_loop_subj <- function(dir = mode.classif, root = path.root, folder, condition, diag = F, crop=T) {
  
  # Individual scores
  if (dir == "results") {
    array.scores <- array(dim = c(141,141,n.subj))
  } else if (dir == "results_v3") {
  ...
  t_train_offset <- case_when(dir == "results" ~ 700,
                           dir == "results_v3" ~ 0)
  t_test_offset <- case_when(dir == "results" ~ 700,
                           dir == "results_v3" ~ 670)
...
cluster_loop_cond <- function(df, H0 = 0, n_permutations = 2**12, n_jobs = -1, crop = FALSE) {
...
# GGPLOT theme
theme_update(plot.tag.position = c(0, 0.95),
...
for (manip in c("","_reord")) { 
  ...
  for (direction in c("rd_to_rd", "rd_to_mm", "rd_to_mp", "rd_to_or")) {
    ...
    tmp <- load_loop_subj(mode.classif = "results",
                          root = "./data/results",
                          folder = "reorder_random_omission",
                          condition = paste0(direction, manip))
...
## Prepare input data
df.tmp <- bind_rows(df.stim.subj, df.omit.subj) %>% # df.omit.subj
...
df.stim.ml.mean %>% filter(manip == "_reord")
...
df.clusters.ml <- tmp.diff %>% #bind_rows(tmp.diff, tmp.ave) %>% 
  cluster_loop_cond(n_permutations = 10000, crop = TRUE) %>% 
  # --- convert indices to time
  mutate(t_test = (dt*(t_test-1)-time_offset+340)) %>% 
  mutate(t_train = (dt*(t_train-1)))
...
df.plot.ml.corr <- df.corr.ml.mean %>% 
  filter(between(t_test,35,68)) %>% mutate(t_test = t_test - 34) %>% 
...
for (manip in c("","_reord")) { 
  ...
  for (direction in c("rd_to_rd", "rd_to_mm", "rd_to_mp", "rd_to_or")) {
    tmp <- load_loop_subj(root = "./data/results/",
                          dir = "results",
                          folder = "reorder_random",
                          condition = paste0(direction, manip),
                          crop = FALSE)
...
for (s in list.subj) {
    ...
    # --- load the confusion matrix
    tmp.file <- os$path$join("./data/results", s, "reorder_random", "rd_to_rd_confmats.npz") %>%
      np$load()
