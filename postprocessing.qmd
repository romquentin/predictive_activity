...
mode.classif <- "results_v3"
path.root <- "./data/results_v3"
list.subj <- list.dirs(path.root, full.names = F, recursive = F)

path.confmat <- "./data/results"
...
```{r}
load_loop_subj <- function(folder, condition, diag = F, crop=T) {
  
  # Individual scores
  if (mode.classif == "results") {
    array.scores <- array(dim = c(141,141,n.subj))
  } else if (mode.classif == "results_v3") {
    array.scores <- array(dim = c(33,135,n.subj))
  }
  
  i <- 1
  for (s in list.subj) {
    file <- os$path$join(path.root, s, folder,
                         paste0("newcv_", condition, "_scores.npy"))

    # Proceed to next subject if file doesn't exist
    if (!os$path$isfile(file)) {next}
  ...
  t_train_offset <- case_when(mode.classif == "results" ~ 700,
                           mode.classif == "results_v3" ~ 0)
  t_test_offset <- case_when(mode.classif == "results" ~ 700,
                           mode.classif == "results_v3" ~ 670)
...
for (manip in c("","_reord")) { 
  ...
  for (direction in c("rd_to_rd", "rd_to_mm", "rd_to_mp", "rd_to_or")) {
    tmp <- load_loop_subj(folder = case_when(mode.classif == "results" ~ "reorder_random",
                                             mode.classif == "results_v3" ~ ""),
...
time_offset <- case_when(mode.classif == "results" ~ 700,
                         mode.classif == "results_v3" ~ 670)
...
for (entropy in c("RD","MM","MP","OR")) {
  ...
  for (s in list.subj) {
    ...
    tmp.file <- os$path$join(path.confmat, s, "reorder_random", "rd_to_rd_confmats.npz") %>%
      np$load()
...
if (save) {
  g %>% ggsave(filename = "./figures/supfig2_MattersArising.png", width = 7, height = 5.5, dpi = 300, device = ragg::agg_png)
}
```
...
```{r}
df.supfig1.mean %>% mutate_if(is.character, as.factor) %>% summary
...
df.supfig1.mean %>% 
  ...
  coord_cartesian(expand = F)-> g

g

if (save) {
  g %>% ggsave(filename = "./figures/supfig1_MattersArising.png", width = 4, height = 2.5, dpi = 300, device = ragg::agg_png)
}
...
tic()
bf.out <- apply(X, c(1,2), getBF01, simplify = TRUE)
toc() # 25s for one array


## Post-processing
df.plot <- left_join(
  # --- append correlation clusters
  matrix_to_df2(bf.out, "t_train", "t_test", "BF"),
  filter(df.clusters.corr, manip == "original", stim == "sounds")) %>% 
  # --- convert indices to time
  mutate(t_test = (dt * (t_test - 1) - time_offset)) %>% 
  mutate(t_train = (dt * (t_train - 1))) %>% 
...
df.plot %>%
  ...
  scale_fill_manual(values = rev(col.pal.bf),
                    ...
                    drop = FALSE) +
  # scale_fill_stepsn(colors = col.pal.bf,
  #                   na.value = "grey50",
  #                   breaks = c(-9,-7,-5,-3,0,3,5,7,9),
  #                   labels = ~case_when(.>0 ~ as.character(.),
  #                                       .==0 ~ "",
  #                                       .<0 ~ paste0("1/",-.)),
  #                   limits = c(-10, 10),
  #                   oob = scales::squish) +
  ...
  labs(x = "Test time (ms)", y = "Train time (ms)") -> g
...
if (save) {
  g %>% ggsave(filename = "./figures/supfig4_MattersArising.png", width = 7, height = 2.5, dpi = 300, device = ragg::agg_png)
}
```
